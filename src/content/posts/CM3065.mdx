---
title: 'CM3065 (ISP) Course Notes'
description: 'Course notes for CM3065: Intelligent Signal Processing third year course at the University of London'
pubDate: 'Oct 18 23'
heroImage: '/uol-logo.svg'
---
import Latex from '../../components/Latex.astro'

# Overview

The course's aims are to teach the basic principles and tools used in intelligent signal processing systems and what _is_ an intelligent signal processing system.  
Intelligent signal processing involves capturing, storing and playing, processing, and extracting information from various signals found in the world using a computer system.
The course will cover how audio and video signals can be captured and processed by a computer program. How you can also extract information from audio and video signals and use this knowledge to implement intelligent signal processing systems, such as speech recognition system or movement and face detection systems.

# Module goals and objectives

Upon successful completion of this module, you will be able to:

1. Explain how audio and video signals can be represented digitally and what the key properties of these signals are.
2. Write computer programs that can capture, process and play back audio and video signals.
3. Understand and use discrete Fourier transforms to process audio signals in the frequency domain and explain the process of feature extraction.
4. Implement simple movement and face detection systems that work with live camera input.
5. Describe the key components of a speech recognition system and analyse the design decisions involved.
6. Select and describe appropriate techniques for compressing audio and video signals

# Topics

1. Digitising, representing and storing audio signals

> Key concepts:
> - Audio fundamentals
> - Digitising audio signals
> - Audio editors: Introduction to Audacity
> - Audio programming with p5.js
> - Introduction to audio programming with Python

2. Editing and processing digital audio

> Key concepts:
> - Digital audio editing
> - Processing digital audio
> - Advanced audio processing

3. Frequency domain representations

> Key concepts:
> - Sound analysis
> - Convolution and the frequency domain

4. Extracting features from audio signals

> Key concepts:
> - Audio signal feature extraction
> - Real-time audio visualizations with Javascript
> - Audio signal feature extraction with Python

5. Speech recognition

> Key concepts:
> - Introduction to speech recognition
> - Speech recognition in p5.js
> - Speech recognition in Python

6. Capturing, representing and processing camera input

> Key concepts:
> - Introduction to digital video
> - Capturing and processing digital video
> - Capturing and processing video with p5.js

7. Computer vision: movement detection

> Key concepts:
> - Introduction to Computer vision
> - Computer vision: movement detection
> - Movement detection with p5.js

8.  Computer vision: face detection

> Key concepts:
> - Computer vision: face detection
> - Face detection with p5.js

9. Audio file formats. Compressing audio signals

> Key concepts:
> - Audio file formats
> - Compressing audio signals
> - Compressing audio signals: software. Compressing audio with python

10. Video file formats. Compressing video signals

> Key concepts:
> - Video file formats
> - Compressing video signals
> - Record, convert and stream audio and video with ffmpeg
> - p5.js and python applications with ffmpeg

# Assessments
- Midterm coursework (multiple submissions throughout the year) (50%)
- Final coursework (multiple submissions throughout the year) (50%)

## Digitising, representing and storing audio signals - part 1

### what is sound?

How is it generated?  
How does it travel?  
How is it perceived?

_In terms of physics:_  
Sound is a form of energy produced by vibrating matter, for example, a bell. Sound is mechanical energy that needs a medium to propagate.  
In contrast to electromagnetic energy, sound cannot travel in vacuum. Sound can travel through solids, liquids, and gases. It travels in the form of mechanical waves, which can move in all directions.

_In terms of physiology and psychology:_  
Sound is the reception of these sound waves and their perception by the brain.  
Sound waves arrive through the ears. Our receptor, a human for example, then converts these waves into sound. The human ear is a complex bio-mechanical system. Sound waves travel up to the ear canal, and force the eardrum to vibrate, then a complicated mechanism converts the vibration to nerve impulses, which are interpreted by the brain of a person or animal, as sound.

### Characteristics of sound waves

- velocity
- wavelength
- frequency
- time period
- amplitude

##### velocity

The speed of sound depends on the elasticity, density, and temperature of the medium the sound is traveling through. e.g. at 0&#176;C, the speed of sound in air is about 331m/s.  
Here, the temperature is important. The density of the material changes with temperature, the greater the density of a material, the higher the speed of sound. For example, at 20&#176;C, the air is more dense than at 0&#176;C and the sound travels faster through it. In particular, the speed of sound in air 20&#176;C is about 343 m/s.

##### wavelength

If we closely observe what happens to the air molecules when we play a bell, we will see that the vibration of the bell produces areas in which the particles are closer together and areas in which they are further apart. In other words, the vibration of the bell creates in the surrounding area a series of alternating high pressured regions called compressions and low pressure regions called rarefactions that travel away from the bell at a certain speed.  
It's interesting to note that the air molecules vibrate back and forth, but they _<u>do not travel with the wave</u>_. Sound waves _transfer energy but not matter_. The wave _energy_ travels in this direction, <mark>not the matter</mark>.  
Sound waves can be represented as a function, which ranges over particle density or pressure values, across the domain of distance.

![wave-diagram](../../../public/wave-diag.png)

This way of representing sound waves helps to introduce some fundamental characteristics of sound:

> __Wavelength__  
> The wavelength can be defined as the distance between two successive crests or troughs of a wave

> __Amplitude__  
> the amplitude is the maximum change in pressure or density that the vibrating object produces in the surrounding air  

Pressure is measured in pascals. Although for practical reasons for measuring sound amplitude, we usually use a logarithm ratio scale, the dB SPL scale.

> __Frequency__  
> The frequency of a wave refers to the number of compressions or rarefactions that pass a given point per unit of time

Frequency is measured in hertz (Hz)

> __Time Period__  
> The time period is the time a sound wave takes to go through a compression-rarefaction cycle

##### relationships

The period (<Latex formula='T'/>) is the inverse of frequency (<Latex formula='f'/>)

<Latex formula='T=\frac{1}{f}, f=\frac{1}{T}' centered={true} />

There is also a direct relationship between sound speed (<Latex formula='v'/>), wavelength (<Latex formula='\lambda'/>) and frequency (<Latex formula='f'/>):

<Latex formula='v=f\lambda' centered={true} />

### Human sound perception

The table below summarizes the physical properties of sound waves: frequency, amplitude, waveform, wavelength, time period, and duration.  
Also the three-dimensions of our psychological sensation of sound: pitch, loudness, and timbre

|Physical properties|Perceptual properties|
|:---:|:---:|
|Frequency|Pitch|
|Amplitude|Loudness|
|Waveform|Timbre|
|Wavelength|
|Time Period|
|Duration|

Pitch is the quality that makes it possible to classify sounds as higher and lower.  
Loudness is the quality that makes it possible to order sounds on a scale from quiet to loud.  
Timbre, also known as tone color or tone quality, describes those characteristic of sound which allow the ear to distinguish sounds which have the same pitch and loudness.

Simplifying, we could say that:
- the frequency of sound waves is related to the perception of pitch
- the amplitude of sound waves is related to their perception of loudness
- the waveform is related to their perceptual of timbre

But this is not so simple. Firstly, the relationship between the physical properties of a sound wave and the way we perceive it is _non-linear_. e.g. a change in frequency does not always correspond with that constant change in pitch.  
Secondly, the way all these properties are related to each other is not so simple. For example, yes, we can say that frequency is related to pitch, but the frequency also affects the loudness and timbre, the amplitude affects the pitch, the waveform affects to the pitch, the duration affects to the pitch and the timbre. In fact, all these properties are related to each other.

##### Pure Tone

A pure tone is a sound that can be represented by a sinusoidal waveform

![sinu-wave](../../../public/sinu-wave.png)

It's important to note that real-world sounds are not pure tones. Pure tones can only be produced technologically. When we play for example, a piano note, we generate that sound with a _fundamental frequency_, but at the same time, we also generate an _infinite number of overtones_. Real-world sounds are complex, composed of many frequencies and a pure tone is a _sound with a single frequency_.

##### Perception of pitch

As has been mentioned previously, frequency is perceived by humans as pitch i.e. a high frequency sound wave corresponds to a high pitch sound, and a low frequency sound wave corresponds to a low pitch sound.

![frequency pitch relationship](../../../public/freq-pitch-rel.png)

As we can see in the above figure, the relationship between pitch and frequency is not a simple linear one. e.g. for frequencies above one kilohertz, a greater change in frequency is needed to produce a corresponding change in pitch

Humans cannot hear all the sound waves that arrive to their ears. For normal hearing the limit of human hearing for frequency falls between 20Hz and 20,000Hz.  
Frequencies above 20,000Hz are known as ultrasounds. Frequencies below 20Hz are known as infrasound.

##### Perception of loudness

The perception of loudness is related to the amplitude of sound waves

| Source|Sound Pressure(uPa)|
| :---: | :---: |
| hearing threshold| 20uPa |
| quiet bedroom at night| 630uPa |
| conversational speech (1m away) | 20,000uPa |
| vacuum cleaner (1m away) |  63,000uPa |
| city road (5m away) | 200,000uPa (0.2Pa) |
| threshold of discomfort | 20,000,000uPa (20Pa) |
| threshold of pain | 63,200,000uPa |
| jet aircraft (1m away) | 632,000,000uPa |

As can be seen, the human ear can perceive a wide range of sound pressure. To express sound amplitude in terms of pascals is inconvenient because we have to deal with numbers from as small as 20 to as big as 20 million.  
For practical reasons, we usually use a logarithmic scale for measuring sound amplitude. The scale that we use is called the __dB SPL scale__.  
The formula that allows us to express sound pressure in terms of dB SPL is:

<Latex formula='SPL=20 \log _{10} (\frac{P}{P_{ref}})' centered={true} />

> here <Latex formula='P' /> is the sound pressure we are measuring, and <Latex formula='P_{ref}' /> is our reference, the pressure of the smallest sound we can hear, <Latex formula='2.0 \times 10^{-5}' /> Pa

The logarithmic scale is useful as it enables the huge range of human hearing from the quiet sound to hearing discomfort to be expressed on a scale of 0-120 dB SPL. 

Another useful aspect of using a decibel scale for expressing sound amplitude is that it gives a much better approximation to the human perception of relative loudness than the pascal scale. The relation between the subjective quality of loudness and the physical quantity of sound pressure level is quite complex, but a couple of important things to note are:

- the ear is more sensitive to high-mid frequencies that to bass frequencies
- the human ear interprets changes in loudness within a logarithmic scale

##### Perception of timbre

Timbre or tone quality is what differentiates two sounds of the _same frequency and amplitude_.  
The perceptual property of timbre is related to the _physical property_ or the <mark>shape of the wave</mark> or the wave form. Timbre is also related to the physical property of the sound's __spectrum__, but this will be covered in a later lecture.

![waveforms](../../../public/waveforms.png)

### The digital world: sample rate and bit depth

##### Capturing and digitising sound waves

We've seen that sound waves can be represented as a function across the domain of time. If we have a sound wave, we can measure with a sound level meter, the air pressure changes in a single point in the air through time.  
In actual fact, the sound level meter uses a microphone to convert air pressure changes into an electrical signal measured in Volts, and this is the first step to capture and digitize a sound wave. The microphone converts air pressure changes into an electrical signal.  
The electrical energy generated by a microphone is usually quite small, so we need a device called __preamplifier__ to convert the weak electrical signal generated by the microphone into an output signal strong enough to be digitized.  
Then the analog-to-digital converter (ADC) measure the incoming voltage a number of times per second and assigns a numerical value to it. We can then use these values in our computer, phone, digital recorder etc.

##### Playing audio

In the opposite case, i.e. we want to play digital audio, we have to use a digital-to-analog converter (DAC). The digital-to-analog converter converts the digital signal back into an analog electrical signal. Then again, we have to use an amplifier to amplify the level of the signal and send it to a speaker or headphones that will generate the sound wave.

##### Sampling rate

The ADC measures the voltage some number of times per second. This number is called the sampling rate, and each measurement of the waveform's amplitude is called a sample. The faster we sample, the better the quality. The sampling rate of a CD is 44.1 kKz, and DVD is 48 kHz. Professional audio can be 88-96 kHz. The __more samples we take, the more memory size we need__

##### The Nyquist-Shannon sampling rate

Nyquist frequency = <Latex formula='\frac{1}{2} \times' />sampling rate

Signals above the Nyquist frequency are not recorded properly by ADCs, introducing artificial frequencies in a process called __aliasing__.  
The sampling rate must be a least twice the frequency of the signal being sampled.

The example below illustrates a 60Hz original track sampled at 50Hz (red dots). <mark>The resultant recording is a 10Hz signal!</mark>

![nyquist-shannon](../../../public/nyquist-shannon-sample.png)

If we want to capture 60 hertz signal, the sampling rate has to be at least 120Hz. This way, we will take enough samples to capture the original signal.

To mitigate the aliasing problem mentioned above, devices add in an 'anti-aliasing filter' before the ADC. This is a low-pass filter that eliminates frequencies above the Nyquist frequency before audio reaches the ADC.

##### Bit depth

> __Sampling Rate__  
> The number of samples taken per second

> __Bit Depth (a.k.a Sample Width, Quantisation level)__  
> The number of bits used to record the amplitude measurements

The more bits used, the more accurately an analogue waveform can be measured, however this means more memory used. Common bit widths used for digital sound representation include: 8, 16, 32 & 64 bits.

![1-bit-res](../../../public/1-bit-res.png)

![1-bit-res](../../../public/16-bit-res.png)

A bit depth of 16 is the mimimum that should be used really for a good audio recording, though 24 or 32 bits can also be used.

##### Clipping

Clipping occurs when the level of the input signal is too high and the ADC cannot assign to the signal the correct measurements. The ADC assigns maximum or minimum amplitude values to many samples in a row. This results in a waveform with a flat top and bottom that generates digital distortion.

![clipping](../../../public/clipping.png)

To avoid this, e.g. whilst using our audacity or another software, the level meters need to be observed to make sure that the input level doesn't reach zero. If the level meter reaches zero, this means that the signal is clipping

> __Practice Question__  
> What is approximately the size of an uncompressed stereo audio file, the sound time of which is one minute at a sampling frequency of 44.1 kHz and a resolution of 16 bits?

freq = 44,100Hz  
bit depth = 16  
length = 60s  
channels = 2 (stereo)

size = <Latex formula='\frac{44,100 \times 60 \times 2 \times 16}{8}=10,584,000 \approx' />10Mb

### Digital audio representation: the time domain

There are two ways of representing digital audio:
- time domain
- frequency domain

The time-domain representation gives the amplitude of the signal at the instance of time at which it was sampled. Plotting these values creates the signal waveform. That's why this way of representing digital audio is also called waveform view.  
For a time-domain representation, time can be expressed in decimal format, samples, frames-per-second etc.  
Amplitude values can be expressed in normalised values (1.0 -> -1.0), decibels (dBFS), sample values (0-65,535) etc.

<mark>__Note__</mark> dBSPL <Latex formula='\neq' /> dBFS

> SPL = Sound Pressure Level

> FS = Full Scale

##### normalised to dBFS

<Latex formula='dBFS=20 \log _{10}(abs(norm\_val) )' centered={true} />

e.g. normalised value = 0 <Latex formula='\rarr dBFS=20 \log _{10}(abs(0) ) = - \infty' />  
e.g. normalised value = 1 <Latex formula='\rarr dBFS=20 \log _{10}(abs(1) ) = 0' />  
e.g. normalised value = -1 <Latex formula='\rarr dBFS=20 \log _{10}(abs(-1) ) = 0' />  


##### Brief intro to frequency domain

There is another way that show the __energy__ of a sound. As eluded to earlier, sounds are complex, they don't just have one frequency. Sound can be composed of millions of frequencies. The frequency domain gives us information about the frequencies of a sound.

![frequency representation](../../../public/freq-rep.png)

##### Spectrogram

A spectrogram is very similar to the frequency domain representation, but here it also visualises how the spectrum changes through time

![spectrogram representation](../../../public/spectrogram-rep.png)

## Digitising, representing and storing audio signals - part 2

### Audio editors: intro to audacity

The first lecture shows how to download and install audacity and takes a tour through it's features and how to use it. Relevant links are here:  
[Home page](https://www.audacityteam.org/)  
[Download page](https://www.audacityteam.org/download/)  
[Manual](https://manual.audacityteam.org/)  
[Tutorials](https://manual.audacityteam.org/#tutorials)  

### Audio programming with p5.js

To use sound with the p5.js library, you need to include the p5.sound library also:  
[p5.sound page](https://p5js.org/reference/#/libraries/p5.sound)

The p5 sound library builds on top of the [web audio api](https://www.w3.org/TR/webaudio/), a high level js api for processing and synthesising audio in web applications.

see code snippets below from lecture for loading and playing audio files:

##### loading and playing back audio files

```javascript
let mySound;
let playStopButton;

function preload() {
  soundFormats('mp3', 'wav');
  mySound = loadSound('/sounds/apollo13');
}

function setup() {
  createCanvas(400, 200);
  background(180);

  playStopButton = createButton('play');
  playStopButton.position(20, 20);
  playStopButton.mousePressed(playStopSound);
}

function playStopSound() {
  if (mySound.isPlaying()) {
    mySound.pause()
    playStopButton.html('play');
  } else {
    mySound.loop()
    playStopButton.html('pause');
  }
}
```

##### playback controls

```javascript
let mySound;
let playStopButton;
let sliderVolume;
let sliderRate;
let sliderPan;
let jumpButton;

function preload() {
  soundFormats('mp3', 'wav');
  mySound = loadSound('test');
}

function setup() {
  createCanvas(400, 200);
  background(180);

  // setup the UI
  playStopButton = createButton('play');
  playStopButton.position(200, 20);
  playStopButton.mousePressed(playStopSound);

  jumpButton = createButton('random jump');
  jumpButton.position(250, 20);
  jumpButton.mousePressed(randomJumpSound);

  sliderVolume = createSlider(0, 2, 1, 0.01);
  sliderVolume.position(20, 25);
  text('volume', 80, 20);

  sliderRate = createSlider(-2, 2, 1, 0.01);
  sliderRate.position(20, 70);
  text('rate', 80, 65);
  
  sliderPan = createSlider(-1, 1, 0, 0.01);
  sliderPan.position(20, 115);
  text('pan', 80, 110);
}

function draw() {
  mySound.setVolume(sliderVolume.value());
  mySound.rate(sliderRate.value());
  mySound.pan(sliderPan.value());
}

// callback for play/pause button
function playStopSound() {
  if (mySound.isPlaying()) {
    mySound.pause()
    playStopButton.html('play');
  } else {
    mySound.loop()
    playStopButton.html('pause');
  }
}

// callback for random jump button
function randomJumpSound() {
  const dur = mySound.duration();
  const t = random(dur);
  mySound.jump(t);
}
```

##### Capture and record audio
```javascript
let mic;
let recorder;
let soundFile;
let state = 0;

function setup() {
  createCanvas(400, 400);

  mic = new p5.AudioIn();
  mic.start();

  recorder = new p5.SoundRecorder();
  recorder.setInput(mic);

  soundFile = new p5.SoundFile();
}

function draw() {
  // background(200);
  let vol = mic.getLevel();

}

function mouseClicked() {
  if (getAudioContext().state !== 'running') {
    getAudioContext().resume();
  }
  
  if (state === 0 && mic.enabled) {
    background(255,0,0);
    text('Recording', 40, 40);

    recorder.record(soundFile);
    state++;

  } else if (state === 1) {
    background(0,255,0);
    text('Click to play and download!', 40, 40);

    recorder.stop();
    state++;

  } else if (state === 2) {
    background(200);
    text('Click to record', 40, 40);

    soundFile.play();
    save(soundFile, 'output.wav');

    state = 0;
  }
}
```

## Editing and processing digital audio - part 1

### Audio editing in Audacity

The first lecture video of the week covers how to:
- zoom
- cut/delete
- trim
- silence
- split
- move
- copy and paste
- envelope

audio in Audacity.

###### Click and pop removal

read [this resource](https://manual.audacityteam.org/man/tutorial_click_and_pop_removal_techniques.html) on click and pop removal from audio.

### Introduction to audio processing

MYK generates a white noise signal for use in the demonstrations. This is due to the fact that white noise is a rich signal. It's got all possible frequencies within the band included, but randomly played at different times.  
In the spectrum view in audacity, the white blobs correspond to power output, so the more white blobs the more power in the track. 

##### Amplitude control

This applies a scalar value to the amplitude values of the portion of the track selected, which in turn scales the amplitude of the track. Note that if you scale the track too high, clipping will occur.

##### Normalisation

Normalization in audio signals is slightly different from a statistical normalization. It's simpler. There are two phases to audio normalisation:

1. analysis
2. scaling

Initially, the signal is analysed to find the peak, or the loudest sample in the whole signal. Then once the highest sample is found, the scaling value is calculated in accordance with a maximum amplitude value supplied, that avoids the occurrence of clipping. It's important to note that the signal gain is proportional across the entire track.

###### Fading in and out

The fade-in/out effect gradually increases/decreases the volume until it hits the maximum/minimum. The fade-in effect is essentially a function which iterates over the values in the selected range and applies an increasingly large scalar starting at zero.

### A range of audio effects

##### Filters

Filters are very commonly used effects that carve out sections from the frequency spectrum.  
Low-pass filters allow lower frequencies to pass, blocking higher frequencies.  A high-pass filter is the opposite. A notch filter cuts out a small section of the frequencies:

![filters](../../../public/assets/cm3065/filters.png)

A low-pass filter makes the sound seem more 'muffled', whereas a high-pass filter makes the sound seem 'tinnier'. A notch filter seems to 'hollow out' the sound.

There is also a band-pass filter, which is a combination of a low-pass filter and a high-pass filter, and together they achieve the opposite of what a notch filter does, it that it allows only a certain band through, and blocks all above and below it.

properties of filters:
- cutoff or centre frequency (where the filter starts taking effect)
- Q (controls how wide the notch is)

##### Delays

###### Echo

An echo repeats the signal. An audacity you can set the delay time and the decay factor.

###### Phasing

Phasing overlays the signal on itself, but with a slight delay. The signal then interferes with itself to create the effect. 

##### Dynamic range

THe key idea is that there is a dynamic way of dealing with the amplitude of the signal. 

###### Gate

Gate blocks the signal when quiet. A way to remember is that if the signal is too quiet then a 'gate closes'. The main properties of gate are threshold and level reduction. Threshold is the level at which the effect is applied, and the level reduction is what the signal is reduced to if it is applied.

###### Compression

Some recordings fluctuate from low to high volume levels, and normalisation can't rectify the signal sufficiently. A dynamic range compression effect detects quiet segments and boosts their volume, while it leaves louder portions unaffected.  
You can set a gain profile in Audacity to specify how the compression is applied:

![compression profile](../../../public/assets/cm3065/compression-profile.png)

##### Waveshaping and distortion

Wave shaping is when you decide what to do based on the current value of the waveform. Distortion is a type of wave shaping that adds extra harmonics on top of the original signal:

![distortion](../../../public/assets/cm3065/distortion.png)

##### Spatialisation and reverberation

Reverberation is concerned with simulating a particular acoustic space. Imagine a large hall, church or a cave, an how sound is effected by the environment. Reverberation tries to recreate those spaces in an artificial way. It's quite a sophisticated effect, and there's been many ways of implementing it over the years. The most advanced form of reverb used today is the convolution reverb.

### Processing digital audio in p5.js: The p5.Effect class

below is a snippet that applies a reverb effect to a sound in p5.js:

```javascript
let player;
let reverb;

function preload() {
  player = loadSound('./path/to/sound.wav');
  reverb = new p5.Reverb();
}

function setup() {
  let canvas = createCanvas(200, 200);
  background(0, 255, 0);
  canvas.mousePressed(mousePressed);
  // disconnect the unchanged signal from the audio output
  player.disconnect();
  // play the signal with the reverb effect applied
  reverb.process(player, 2, 3);
}

function mousePressed() {
  player.loop();
}

```

## Editing and processing digital audio - part 2

### Filtering, and an implementation of an averaging low-pass filter

Signal averaging is a signal processing technique that tries to remove unwanted random disturbances from a signal through the process of averaging.  
Averaging often takes the form of summing a series of signal samples and then dividing that sum by the number of individual samples.  
The following equation represents an N-point moving average filter, with input array `x` and output averaged array `y`:

<Latex formula="y(N) = \frac{1}{N} \cdot \displaystyle \sum_{k=0}^{N-1} x_{N-k}" centered={true} />

Let's see an example. Let's apply a 3-point moving average filter example. In this example, the input is the array `values` and the averaged output the array `averages`:

```python
import numpy as np
import matplotlib.pyplot as plt

values = np.array([3., 9., 3., 4., 5., 2., 1., 7., 9., 1., 3., 5., 4., 9., 0., 4., 2., 8., 9., 7.])
N = 3

averages = np.empty(20)
for i in range(1, 19):
    temp = (values[i-1]+values[i]+values[i+1])/N
    averages[i] = temp

# Preserve the edge values
averages[0] = values[0]
averages[19] = values[19]  

plt.plot(values, 'b-', label='values')
plt.plot(averages, 'r--', label='averages')
plt.legend(loc="upper left")
```

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![moving average](../../../public/assets/cm3065/moving-avg.png)
  </div>
</div>

We can also use __numpy.convolve__ or __scipy.ndimage.uniform_filter1d__ to implement the moving average filter.

This is an example using __numpy.convolve__:

```python
window = np.ones(3)
window /= sum(window)

averages = np.convolve(values, window, mode='same')
plt.plot(values, 'b-', label='values')
plt.plot(averages, 'r--', label='averages')
plt.legend(loc="upper left")
```

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![moving average](../../../public/assets/cm3065/moving-avg-conv.png)
  </div>
</div>

This is an example using __scipy.ndimage.uniform_filter1d__:

```python
from scipy.ndimage.filters import uniform_filter1d

averages = uniform_filter1d(values, size=3)
plt.plot(values, 'b-', label='values')
plt.plot(averages, 'r--', label='averages')
plt.legend(loc="upper left")
```

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![moving average](../../../public/assets/cm3065/moving-avg-1d-filt.png)
  </div>
</div>


notice the start and finish values are averaged differently with the different methods.

#### Implementation of an averaging low-pass filter

In signal processing, the moving average filter can be used as a simple low-pass filter. The moving average filter smooths out a signal, removing the high frequency components from it, and this is what a low-pass filter does.  
In this example, we are going to apply a low-pass filter (a 15-point moving average filter) to a square wave:

```python
import numpy as np
import matplotlib.pyplot as plt
from thinkdsp import SquareSignal, Wave

# Suppress scientific notation for small numbers
np.set_printoptions(precision=3, suppress=True)

# make a square signal wave
signal = SquareSignal(freq=440)
wave = signal.make_wave(duration=1.0, framerate=48000)

# make a moving average window
window = np.ones(15)
window /= sum(window)

# apply the window to the signal using np.convolve
averages = np.convolve(wave.ys, window, mode='same')
smooth = Wave(averages, framerate=wave.framerate)

# plot both
plt.plot(wave.ys[:500],'b-', label='original')
plt.plot(smooth.ys[:500], 'r--', label='filtered')
plt.xlabel('Samples')
plt.legend(loc="upper left")
```

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![moving average](../../../public/assets/cm3065/smoothed-wave.png)
  </div>
</div>

below is the jupyter notebook from the lab:  
<mark>N.B. turn speakers down before playing sound!</mark>

<iframe
  src="/assets/cm3065/filtering.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### FIR (Finite Impulse Response) filters

In signal processing, a finite impulse response (FIR) filter is a filter whose impulse response (or response to any finite length input) is of finite duration, because it settles to zero in finite time.

For a general N-tap FIR filter, the nth output is:


<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    <Latex formula="y(N) = \displaystyle \sum_{k=0}^{N-1} h(k) \cdot x_{N-k}"  />
  </div>
  <div>
    <Latex formula="h(k) = \frac{1}{k}, 0 \le k \lt N" />
  </div>
</div>

example below:

```python
import numpy as np
import matplotlib.pyplot as plt
from thinkdsp import SquareSignal, Wave, decorate
from thinkdsp import read_wave

# the wave to filter
my_sound = read_wave('../Audio/429671__violinsimma__violin-carnatic-phrase-am.wav')
my_sound.make_audio()

# make a 5-tap FIR filter using the following coefficients: 0.1, 0.2, 0.2, 0.2, 0.1
window = np.array([0.1, 0.2, 0.2, 0.2, 0.1])

# apply the window to the signal using np.convolve
filtered = np.convolve(my_sound.ys, window, mode='same')
filtered_violin = Wave(filtered, framerate=my_sound.framerate)
filtered_violin.make_audio()
```

### LTI (Linear Time Invariant) systems

#### What is a signal?
- a list of numbers
- for audio signals, this represents the amplitude sampled over time

#### What is a system?
- something used to process signals
- if it happens to be a linear time invariant system, we can represent its behaviour as a list of numbers known as an __IMPULSE RESPONSE__
- given an impulse response, we can easily process any signal with that system using __convolution__

#### characteristics of LTI systems

Linear systems have very specific characteristics which enable us to do the convolution:
1. homogeneity
2. additivity
3. shift invariance

##### Homogeneity

Homogeneity means 'linear with respect to scale'.  
It is demonstrated in the jupyter notebook below that two signals, both the same signals but one is scaled by a a factor of 0.5, produce the same outputs but one is scaled by a factor of 0.5.

##### Additivity

Additivity means you can separately process simple signals and add the results together.  
For example, you could decompose a signal into four separate sections. If you pass each of those through the system one at a time, then you'd get four outputs which if all added together would return the final output, which would be the result of the undecomposed signal coming through the whole system. So it means you can arrive at the same results even if you process the signal in its individual components.  
This is a very important property for convolution

Again this is shown in the notebook below.

##### Shift Invariance

Shift invariance means later signal -> later response.  
Again this is shown in the notebook below.

### Convolution by hand

steps to convolve by hand:

1. decompose (additivity)
2. scale (homogeneity)
3. shift (shift invariance)
4. synthesise (addition)

##### decompose

imagine a signal:

```
sig = [1.0, 0.5, 1.0]

```

you can decompose it into three parts:

```
a = [1.0, 0.0, 0.0]
b = [0.0, 0.5, 0.0]
c = [0.0, 0.0, 1.0]

```

##### scale

using a system, you can now scale the decomposed constituents:

```
sig = [1.0, 0.5, 1.0]
sys = [0.0, 0.5, 0.25]

a = [1.0, 0.0, 0.0]
a_output = [0.0, 0.5, 0.25]

b = [0.0, 0.5, 0.0]
b_output = [0.0, 0.25, 0.125]

c = [0.0, 0.0, 1.0]
c_output = [0.0, 0.5, 0.25]

```

##### shift

because of the positions of the scalars in the system, the outputs need to be shifter in accordance:

```
sig = [1.0, 0.5, 1.0]
sys = [0.0, 0.5, 0.25]

a = [1.0, 0.0, 0.0]
a_output = [0.0, 0.5, 0.25]

b = [0.0, 0.5, 0.0]
b_output = [0.0, 0.0, 0.25, 0.125]

c = [0.0, 0.0, 1.0]
c_output = [0.0, 0.0, 0.0, 0.5, 0.25]

```

##### synthesise

the constituents can now be added back together:

```
a_output = [0.0, 0.5, 0.25]
b_output = [0.0, 0.0, 0.25, 0.125]
c_output = [0.0, 0.0, 0.0,  0.5,  0.25]

output = [0.0, 0.5, 0.5, 0.625, 0.25]
```

### Convolution in python

Below is an implementation of convolution in python, though note that this is far to slow in real life as the computational complexity is too high: 

```python
def convolve(signal, system):
    res = np.zeros(len(signal) + len(system) - 1)
    for sample_ind in range(len(signal)):
        sample = signal[sample_ind]
        for system_ind in range(len(system)):
            output = sample * system[system_ind]
            res_ind = sample_ind + system_ind
            res[res_ind] = res[res_ind] + output
    return res
```

The below notebook shows an example of applying a convolution to MYK's voice recording using an impulse response taken from a recording of a clap being played in St. Alban's. This gives the effect that he's actually in St. Alban's

### LTI and convolution jupyter notebook

<iframe
  src="/assets/cm3065/LTI-systems-2.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### Lab exercises jupyter notebook

<iframe
  src="/assets/cm3065/ex7.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

## Week 5: Frequency domain representations - part 1

### Review of spectral analysis

_What's wrong with the time domain?_

Nothing really for the most part, but the problem lies in the inability to look at something like the time-domain view below:

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![time domain plot](../../../public/assets/cm3065/time-domain.png)
  </div>
</div>

What can you decipher about the sound from this? You can see that it's going to be loud at the beginning, and that it tails off a bit towards the end, but other than that not too much. You can't decipher the pitch for instance. This is where spectral analysis comes in. See below a 2D spectral analysis in Audacity of part of a track:

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![spectral analysis](../../../public/assets/cm3065/spect-anal.png)
  </div>
</div>

On the y-axis is power, and the x-axis is frequency. From this you can glean that there is more power in the lower frequencies of the track.  
The problem with a 2D view is that you can't see how these frequencies change over time. This makes way for 3D spectrograms:

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![3D spectrum](../../../public/assets/cm3065/3D-spect.png)
  </div>
</div>

In this instance, the x-axis is time, the y-axis is frequency, and colour denotes the power. The lighter colour above corresponds to higher power. The genre of the track is techno, and you can see the repeating drum and cymbal pattern from the 3D plot. 

#### Bins and windows

Bins represent a single frequency typically, or a small range of frequencies. A window is a snapshot of frequencies at a moment in time. So a snapshot displays different frequency bins at a moment in time.



### Spectral plots and periodic signals

The notebook below runs through what has just been covered above re: spectral analysis, but also covers periodic signals.  

##### sine wave
- one strong, clear band in the frequency spectrum

##### sawtooth wave
- much busier frequency spectrum
- strong bands at the base frequency, and multiples of that frequency (these are called harmonics)
- buzzing sound, much more stimulating to the ear. This makes sense as the frequency spectrum is much busier than the sinusoidal one

##### square (pulse) wave
- strong bands at the base frequency, and multiples of that frequency, but not as many harmonics as the sawtooth wave
- harmonics are more spaced out than sawtooth
- sound is likened to a 'hollow' sound

##### multiple sine waves added
- as can be seen, adding multiple sine waves of different frequencies together results in a different wave form
- the frequency spectrum of this resembles (also sounds like) a sawtooth spectrum
- this means 
  - you can build sawtooth waves from sine waves
  - the spectrum tells us which sine waves we need to build a signal


#### Fourier decomposition

Fourier decomposition is a formalisation of the process of figuring out how to make a given signal out of a set of sine waves:

<Latex formula="s_N(x) = \frac{a_0}{2} + \displaystyle \sum_{n=1}^{N}(a_n \cdot cos(\frac{2 \pi}{P}nx) + b_n \cdot sin(\frac{2 \pi}{P}nx))" centered={true} />

but more on this later.

### Spectral plots and periodic signals jupyter notebook

<iframe
  src="/assets/cm3065/spps.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### Signal analysis with the DCT

Watch MYK's DCT video, where he runs through the below notebook with explanations:

<iframe
  src="/assets/cm3065/dct.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

## Week 6: Frequency domain representations - part 2

### Complex synthesis

Watch MYK's 'complex synthesis' video, where he runs through the below notebook with explanations:

<iframe
  src="/assets/cm3065/complex-synthesis.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### Complex analysis with the DFT

Watch MYK's 'complex analysis with the DFT' video, where he runs through the below notebook with explanations:

<iframe
  src="/assets/cm3065/complex-analysis-with-the-DFT.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### Windowed DFT

Watch MYK's 'windowed DFT' video, where he runs through the below notebook with explanations:

<iframe
  src="/assets/cm3065/windowed-DFT.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

### Faster convolution with the DFT

Watch MYK's 'Faster convolution with the DFT' video, where he runs through the below notebook with explanations:

<iframe
  src="/assets/cm3065/faster-convolution-with-the-DFT.html"
  width="100%"
  style={{height: '70vh'}}
></iframe>

## Week 7: Extracting features from audio signals - part 1

### Introduction to audio feature extraction

#### What is an audio feature?

An audio feature is a measurement of a particular characteristic of an audio signal, and it gives us insight into what the signal contains. Some audio feature examples are:
- energy
- spectrum
- fundamental frequency
- loudness
- brightness
- pitch
- rhythm
- ...


#### How can audio features be extracted?

Audio features can be measured by running an algorithm on an audio signal that will return a number, or a set of numbers that quantify the characteristic that the specific algorithm is intended to measure. Feature extraction can be 'real-time' or 'non-real-time'. Some audio feature extraction toolboxes include:
- librosa (python)
- YAAFE (C++, can be used in python or matlab)
- Meyda (js, web)

Audio features can be extracted from the time domain representation of audio or the frequency domain representation.

#### Applications of audio features

- real-time audio visualisations
- visual score generation
- speech recognition
- music recommendation
- music genre classification
- feature-based synthesis
- feature extraction linked to audio effects
- ...

#### Time-domain features

- energy
- RMS energy (RMSE)
- Zero rossing rate (ZCR)
- Amplitude envelope (AE) (loudness)
- ...

#### Frequency-domain features

- spectral centroid
- spectral rolloff
- spectral flatness
- spectral flux
- spectral slope
- spectral spread
- mel-frequency cepstral coefficients (MFCC)
- ...

#### RMS energy (RMSE)
- calculates the RMS of a signal or a frame of a signal
- indicator of loudness
- less sensitive to outliers than amplitude envelope
- used for:
  - audio segmentation
  - music genre classification
- the formula that formalises this concept:

<Latex formula="\sqrt{\frac{1}{n} \displaystyle \sum_{n} |x_{(n)}^2|}" centered={true} />

<div style={'display: flex; flex-direction: row; justify-content: space-around; align-items: center;'}>
  <div>
    ![3D spectrum](../../../public/assets/cm3065/rmse.png)
  </div>
</div>

#### Zero crossing rate (ZCR)

- the number of times that the signal crosses the zero value in a frame
- recognition of percussive vs pitched sounds
- used for:
  - monophonic pitch estimation
  - speech recognition for voice activity detection

#### Spectral centroid

- indicates where the spectral 'centre of mass' or 'centre of gravity' of a sound is located
- it is an indicator of 'brightness' of a sound
- e.g. it can be used to classify a bass guitar (low spectral centroid) from a trumpet (high spectral centroid)

__N.B. Watch 3 lecture videos on real-time audio visualisations with p5.js and one with Meyda__  

