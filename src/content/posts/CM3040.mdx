...
### Week 7: Human interaction with IoT devices and wearable sensors
...
### Week 8: Interaction with input/output sensors and automation

#### Introduction to automations and triggers

Automation is generally based on time. We can set timers, reminders, or simply create a sequence of operations to run with a time delay. For example, turning LED on and off, or changing colors with a time delay.  
Triggers are based on specific conditions, or when an event has occurred. In which case, we can add a set of instructions to perform some task. You can view this as an if-else statement. A temperature control system is a typical example of trigger event. If a temperature in a room is less than -1&deg;C, then we can program it to turn the heater on, or if it's less than 18&deg;C, then turn the fan on. Similarly, if the temperature is greater than 25&deg;C, we can turn air con on. Likewise you can use distance sensor, and perform pulse width modulation to set brightness based on how close an object is.

#### Smart fridge part 6

<iframe
  src="../../../src/assets/CM3040/smart-fridge-part6.pdf"
  width="100%"
  style={{height: '70vh'}}
></iframe>

#### Smart door part 3 - trigger LED (PWM) with distance sensor threshold

<iframe
  src="../../../src/assets/CM3040/smart-door-part3.pdf"
  width="100%"
  style={{height: '70vh'}}
></iframe>
